...
class DisplayLib:
    ...
    def initialize(self) -> bool:
        ...
        try:
            ...
            self.current_image = Image.new(
                'RGB', 
                (self.fb_info.width, self.fb_info.height), 
                self.BLACK
            )
    ...
    def _get_framebuffer_info(self) -> bool:
        ...
        try:
            ...
            try:
                ...
                self.logger.info(f"Hardware framebuffer: {self.fb_info.width}x{self.fb_info.height}, "
                ...
                if self.fb_info.line_length == 0:
                    ...
                    self.logger.warning(f"Calculated line_length manually: {self.fb_info.line_length}")
                
                self.fb_info.buffer_size = self.fb_info.line_length * self.fb_info.height
            
            self.logger.info(f"Framebuffer: {self.fb_info.width}x{self.fb_info.height}, "
            ...
            try:
                ...
                self.fb_info.line_length = self.fb_info.width * self.fb_info.bytes_per_pixel
                self.fb_info.buffer_size = self.fb_info.line_length * self.fb_info.height
                self.logger.warning(f"Using fallback framebuffer settings: {self.fb_info.width}x{self.fb_info.height}")
                return True
            except:
                return False
    ...
    def update_display(self) -> bool:
        ...
        try:
            ...
            if fb_bytes_per_pixel == 4:
                ...
                fb_array[:copy_height, :copy_width, 0] = pixels[:copy_height, :copy_width, 2]  # B <- R
                fb_array[:copy_height, :copy_width, 1] = pixels[:copy_height, :copy_width, 1]  # G <- G  
                fb_array[:copy_height, :copy_width, 2] = pixels[:copy_height, :copy_width, 0]  # R <- B
    ...
    def display_image(self, image_path: str, position: Tuple[int, int] = (0, 0),
